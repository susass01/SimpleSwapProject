...
function getAmountOutPublic(
    ...
    require(pair.totalLiquidity > 0, "Pair does not exist");
...
// Devuelve la liquidez total para un par tokenA-tokenB
function getTotalLiquidity(address tokenA, address tokenB) external view returns (uint) {
    bytes32 pairKey = _getPairKey(tokenA, tokenB);
    return pairs[pairKey].totalLiquidity;
}

// Devuelve la liquidez que tiene un usuario para un par tokenA-tokenB
function getUserLiquidity(address tokenA, address tokenB, address user) external view returns (uint) {
    bytes32 pairKey = _getPairKey(tokenA, tokenB);
    return pairs[pairKey].liquidity[user];
}
}